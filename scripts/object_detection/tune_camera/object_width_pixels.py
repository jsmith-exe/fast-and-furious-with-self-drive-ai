'''
Used to determine the width in pixels of the object you are trying to detect.
'''
import cv2
import numpy as np
...
# TUNE HERE: Set the lower and upper HSV bounds for your object color
# Example: For a maroon object, start with these and adjust as needed
...
object_camera = CSICamera(width=224, height=224, capture_fps=30)
bbox_history = collections.deque(maxlen=5)
app = Flask(__name__)

object_camera.running = False
cv2.destroyAllWindows()

def gen_frames():
    print(f"Current HSV bounds: LOWER_HSV={LOWER_HSV}, UPPER_HSV={UPPER_HSV}")
    while True:
        frame = object_camera.read()
        ...
        if contours:
            ...
            if cv2.contourArea(largest) > min_area:
                ...
                if w > 0:
                    # Display pixel width instead of distance
                    cv2.rectangle(annotated, (x, y), (x+w, y+h), (0,255,0), 2)
                    cv2.putText(annotated, f'Width: {w} px', (x, y-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,0), 2)
